...
class LoginViewController: UIViewController, UITextFieldDelegate {
    ...
    lazy var state: LoginViewControllerState = {
        return .login
    }()
    var timer: Timer?
        ...
        if state == .login {
            ...
            if emailField.text?.range(of: "@") != nil {
                if (passwordField.text?.lengthOfBytes(using: String.Encoding.ascii))! >= 6 {
                    LoginManager.login(emailField.text,
                ...
                if emailField.text?.range(of: "@") != nil {
                    if (passwordField.text?.lengthOfBytes(using: String.Encoding.ascii))! >= 6 {
                        if passwordField.text == verfyPasswordField.text {
                            LoginManager.signUp(emailField.text,
                                                password: passwordField.text,
    ...
    @IBAction func forgetPassword(_ sender: UIButton) {
        ...
        alertVC.addAction(UIAlertAction.init(title: "确定", style: .default, handler: { action in
            LoginManager.resetPassword(alertVC.textFields![0].text, success: {
                [weak self] message in
    ...
    @IBAction func sendVerfyCode(_ sender: UIButton) {
        if sender.isEnabled {
            ...
            timer = Timer.scheduledTimer(timeInterval: 1,
                                         ...
                                         userInfo: nil,
                                         repeats: true)
            LoginManager.fetchVerfyCode(phoneField.text,
                                        success: {
            },
                                        failure: {
                                            [weak self] message in
