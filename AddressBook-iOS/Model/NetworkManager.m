...
- (AFHTTPRequestOperation*)DELETE:(NSString *)key
                       parameters:(NSDictionary *)parameters
                          success:(void (^)(NSDictionary *))success
                          failure:(void (^)(NSError *))failure
{
    return [self request:key method:@"DELETE" GETParameters:nil POSTParameters:parameters constructingBodyWithBlock:nil success:success failure:failure];
}
                           ...
                           failure:(void (^)(NSError *))failure
{
    @try {
        ...
        if ([method  isEqual: @"PUT"]) {
            ...
            return [self.manager DELETE:URLString parameters:POSTParameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
                [weakSelf handleSuccess:operation data:responseObject success:success failure:failure];
            } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
                if (failure) {
                    failure(error);
...
- (void)cleanCookies {
    [NetworkManager defaultManager].token = @"";
}
